...
//转码 <highlight></highlight> 之间的代码
function tranCode($content)
{
  //  $content = "<code>第一处</code><code>第二处</code><code>第三处</code>";
    preg_match_all('|\s<highlight>(.*?)</highlight>\s|is', $content, $output);
    $i = count($output[1]);
  for ($n=0;$n<$i;$n++)
  {
    $transformed =htmlspecialchars(trim($output[1][$n]), ENT_QUOTES);
